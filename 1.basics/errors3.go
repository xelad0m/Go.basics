...
func (e invalidStepError) Error() string {
	return fmt.Sprintf("cannot %s", e.st) // fmt.Sprintf("things like '%s %s' are impossible", e.st.cmd, e.st.obj)
}
...
func (e notEnoughObjectsError) Error() string {
	return fmt.Sprintf("there are no %ss left", e.obj) //fmt.Sprintf("be careful with scarce '%s's", e.obj)
}
...
func (e commandLimitExceededError) Error() string {
	return fmt.Sprintf("you don't want to %s anymore", e.cmd) // fmt.Sprintf("%s less", e.cmd)
}
...
func (e objectLimitExceededError) Error() string {
	return fmt.Sprintf("you already have a %s", e.obj) // fmt.Sprintf("don't be greedy, 1 '%s' is enough", e.obj)
}
...
func (p *player) do(cmd command, obj thing) error {
	...
	case eat:
		if p.nEaten > 1 {
			return commandLimitExceededError{cmd} //errors.New("you don't want to eat anymore")
		}
		p.nEaten++
	case take:
		if p.has(obj) {
			return objectLimitExceededError{obj} //fmt.Errorf("you already have a %s", obj)
		}
		p.inventory = append(p.inventory, obj)
	case talk:
		if p.nDialogs > 0 {
			return commandLimitExceededError{cmd} //errors.New("you don't want to talk anymore")
		}
		p.nDialogs++
	}
	return nil
...
func (g *game) execute(st step) error {
	...
	if !st.isValid() {
		return gameOverError{g.nSteps, invalidStepError{st}}//&invalidStepError{st} // fmt.Errorf("cannot %s", st)
	}
	...
	if st.cmd == take || st.cmd == eat {
		if !g.has(st.obj) {
			return gameOverError{g.nSteps, notEnoughObjectsError{st.obj}}//&notEnoughObjectsError{st.obj} //fmt.Errorf("there are no %ss left", st.obj)
		}
...
func giveAdvice(err error) string {
	...
	if errors.As(err, &ise) {
		return fmt.Sprintf("things like '%s' are impossible", ise.st)
	}
	...
	return "because fuck you thats why"
}
